$(document).ready(function() {
	...
	function a(a) {
		if (this.options = i.defaults(this.options, a), this.el = this.options.el,
				delete this.options.el, !this.el)
			throw "No Canvas Selected";
		if (this.ctx = this.el.getContext("2d"), this.waves = this.options.waves,
				delete this.options.waves, !this.waves || !this.waves.length)
	...
	function b(a, b) {
		return i.isType(a, "number") ? a : ( a = a.toString(), a.indexOf("%") > -1 ? ( a = parseFloat(a), a > 1 && (a /= 100), b * a) : a.indexOf("px") > -1 ? parseInt(a, 10) :
			void 0)
	}
	...
	var f = Math.PI / 180, g = 2 * Math.PI, h = Math.PI / 2, i = {}, j = i.isType = function(a, b) {
	...
	var p = {
		...
		segmentLength : 10,
		lineWidth : 1,
		strokeStyle : "rgba(176, 221, 251, 0.5)",
		type : "Sine"
	};
	return a.prototype.getDimension = function(a) {
		return i.isNumber(this.options[a]) ? this.options[a] : i.isFunction(this.options[a]) ? this.options[a].call(this, this.el) : "width" === a ? this.el.clientWidth : "height" === a ? this.el.clientHeight :
			void 0
		...
		var c, d;
		for ( c = 0; c < this.waveWidth; c += b.segmentLength)
			d = this.getPoint(a, c, b), this.ctx.lineTo(d.x, d.y), d =
				void 0;
		c =
			void 0, b =
			void 0, this.ctx.lineTo(this.width, this.yAxis), this.ctx.stroke()
	...
	var a = new SineWaves({
		...
		resizeEvent : function() {
			//function getRgba(arr){//2016-1-15 add colors
			...
			//var colors = [];
			//for (var i = 0 ; i < 3 ; i++){
			//	colors[i]=this.ctx.createLinearGradient(0, 0, this.width, 0);
			//	colors[i].addColorStop(0, getRgba(rgba[i][0]));
			//	colors[i].addColorStop(0, getRgba(rgba[i][1]));
			//	colors[i].addColorStop(0, getRgba(rgba[i][2]));
			//}
			var a = this.ctx.createLinearGradient(0, 0, this.width, 0);
			a.addColorStop(0, "rgba(176, 221, 251, 0.5)"), a.addColorStop(.5, "rgba(176, 221, 251, 0.5)"), a.addColorStop(1, "rgba(176, 221, 251, 0.5)");
